...
#ifdef MAIN
int16_t main_unpack (char *pname, char *iname, char *oname);
int16_t copy_file (char *iname, char *oname);
...
typedef struct {
  ...
  int16_t bitcount;		       /* how many bits does bitbuf hold? */
} bit_stream;

typedef struct {
  int16_t num;			       /* number of nodes in the tree */
  struct {
    unsigned long code;
    int16_t codelen;
    int16_t value;
...
static void bit_advance (bit_stream *bs, int16_t n, unsigned char **p);
static unsigned long bit_read (bit_stream *bs, unsigned long mask,
			       int16_t n, unsigned char **p);
...
const char *rnc_error (long errcode) {
  ...
  if ((unsigned long)errcode > sizeof(errors)/sizeof(*errors) - 1)
...
long rnc_unpack (void *packed, void *unpacked
  ...
  if (rnc_crc(input, inputend-input) != bword(input-4))
    // also happens with old-style RNC packer (no CRC) 
